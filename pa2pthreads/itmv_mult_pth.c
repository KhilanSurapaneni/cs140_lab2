...
static int stop_global = 0;
static int local_stop[THREAD_COUNT_MAX];
...
void work_block(long my_rank) 
{
  ...
  int start = (int)my_rank * blocksize;
  ...
  for (int j = 0; j < no_iterations; j++) 
  {
    ...
    if (my_rank == 0) 
    {
      ...
      for (int i  = 0; i < P; i++) 
      {
        all_stop = all_stop && local_stop[i];
      }
      stop_global = all_stop;
    }
...
void work_blockcyclic(long my_rank) 
{
  ...
  for (int k = 0; k < no_iterations; k++) 
  {
    ...
    for (int j = (int)my_rank; j < num_blocks; j += P) 
    {
      int start = j * B;
      int end = start + B;
      if (end > n)
    ...
    for (int m = (int)my_rank; m < num_blocks; m += P) 
    {
      int start = m * B;
      int end = start + B;
      if (end > n) end = n;
