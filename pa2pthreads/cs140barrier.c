...
int cs140barrier_init(cs140barrier *bstate, int total_nthread) {
  if (bstate == NULL || total_nthread <= 0) return -1;
...
int cs140barrier_wait(cs140barrier *bstate) {
  ...
  boolean this_round = bstate->odd_round;

  // thread has arrived
  bstate->arrive_nthread++;

  // last to arrive does this 
  if (bstate->arrive_nthread == bstate->total_nthread)
   {
    bstate->arrive_nthread = 0;  

    if (bstate->odd_round == False)
    ...
    return 1; // last returns 1
  }

  // others do this 
  while (bstate->odd_round == this_round) 
  {
    pthread_cond_wait(&bstate->barrier_cond, &bstate->barrier_mutex);
  }
  pthread_mutex_unlock(&bstate->barrier_mutex);
  return 0; // other threads return 0
}
...
int cs140barrier_destroy(cs140barrier *bstate) {
  if (bstate == NULL) return EINVAL;

  int x;

  x = pthread_cond_destroy(&bstate->barrier_cond);
  if (x != 0) return x;

  x = pthread_mutex_destroy(&bstate->barrier_mutex);
  if (x != 0) return x;
